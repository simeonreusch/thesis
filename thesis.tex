...
To simplify matters and allow reprocessing, all ZTF alerts received via the Kafka streams since June 2018 are also stored in an archival alert database hosted at DESY\sidenote{\url{https://ampel.zeuthen.desy.de/api/ztf/archive/v3/docs}}. The database is \texttt{Postgresql}\sidenote{\url{https://postgresql.org/}} and can be accessed with a web frontend and an API.

\section{Candidate filtering with \texttt{nuztf}}
To streamline the neutrino follow-up process, \texttt{nuztf} \sidecite{Stein2023} was created for filtering and inspecting candidate counterparts.
