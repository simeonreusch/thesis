...
Today we assume that most galaxies contain a SMBH at their core. If the accretion rate, i.e. the rate at which the SMBH accumulates mass, is high, the galaxy appears active. We also assume that the myriad of different types of active galaxies (e.g. blazars, Seyfert I and II galaxies, radio-loud galaxies, etc.) are appearances of intrinsical similar objects: They are merely a function of the viewing angle at which we look at the AGN system.

As can be seen in Fig. \ref{fig:agn_unification}, these systems are thought to contain a SMBH in the center. This is fed by a surrounding accretion disk of hot material, which in turn is enshrouded in a dusty torus. Perpendicular to the accretion disk, often energetic jets of relativistic material are launched from the system. When one looks directly into the jet, the system appears as \textit{blazar} (top of \ref{fig:agn_unification}). If one moves away from the jet direction, the AGN will in general appear as Type I AGN, displaying broad emission lines stemmming from the so-called \textit{Broad-Line Region} (BLR), lying at a distance of \SIrange{e-2}{1}{\parsec} to the SMBH. It consists of gas clouds, and the Doppler-induced line-broadening suggest high velocities, resulting from the fact that the material is deep within the graviational well of the SMBH.

Moving further down, the BLR close to the core gets hidden behind the dust torus, and only the  \textit{Narrow-Line Region} (NLR) is still visible, as it is more distant from core (\SIrange{e2}{e4}{\parsec}). Also here gas heated by the core radiates, but the gas density is lower compared to the BLR, and from the line-broadening one can infer much lower velocities of \SI{\sim 500}{\kilo\m\per\s} \sidecite{Beckmann2012}.


\begin{figure}[htb]
    \includegraphics[width=0.7\textwidth]{theory/agn_unification.pdf}
    \caption[AGN]{AGN unification scheme. Depending on the viewing angle towards the AGN system, different source classes emerge. Following the argumentation of \cite{Padovani2017}, the upper (lower) segment label `radio loud' (`radio quiet') segment was changed to `jetted' (`non-jetted'). Adapted from \cite{Thorne2022}.}
...
The most promising site of high-energy neutrino production is the relativistic jet. In some cases these can be visually detected, like the prominent jet visible in M87 (see Fig \ref{fig:m87_jet}). Jets are thought to be highly collimated, extremely energetic plasma structures launched from the accretion disk. About \SI{10}{\percent} of AGN show evidence of such jets. When the electrons and protons contained in the jet plasma are spinning in magnetic fields, they emit synchrotron radiation \sidecite{Combes2021}.

These synchrotron photons now self are a target field for their `parent' electrons and protons. If the formerThe former can interact with them via inverse Compton Scattering (CS), transferring energy to those photons. This process is called Synchrotron Self-Compton (SSC) \cite{Spurio2018}, resulting in the typical double humped structure of blazar spectral energy distributions (SED).

\sidecite{Meszaros2015}

\todo{more details on jets}
...
Tidal Disruption Events (TDEs) mark the end of very unlucky stars. When these happen to approach the SMBH at the galaxy center too closely, the resulting gravitation forces might overcome the star's self gravity, tidally disrupting and destroying the star \sidecite{Rees1988}. Roughly half the mass of the star is accreted around the black hole, and the bright electromagnetic flare created by this can shine for months. Nevertheless, the exact mechanisms at play causing this emission are still widely discussed \sidecite{Gezari2021}.
...
TDEs normaly emit a continuum. This can be well approximated by a blackbody. Peculiarly, there seems to be a bimodal distribution of blackbody temperatures: One optical/UV population, and a second population peaking in the X-ray, better described by blackbodies of higher temperature \cite{Gezari2021}. 
...
If the disrupted star's circularizes rapidly, one interesting approach to unify the two populations (X-ray vs. optical/UV) is a viewing angle dependet model, akin to the unified AGN model presented in Section \ref{agn}.
...
Also, about \SI{1}{\percent} of TDEs are expected to launch relativistic jets, like e.g. the recently discovered AT2022cmc \sidecite{Andreoni2022}. Such jets (denoted as (1) in Fig. \ref{fig:tde_unification}), as well as the possible winds/outflows (2) or a potentially present disk corona (3) have been proposed as sites where high-energy neutrinos (shown as green lines) could be produced (see e.g. \sidecite{Murase2020} for the non-jet production sites).

\section{Current sources and limits}
\subsection{AGN}
In blabla, IceCube\sidenote{See Chapter \ref{ic}.} identified the flaring blazar TX0506+056 as the probable source of high-energy neutrino b
%\sidecite{Murase2020}
