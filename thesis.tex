...
\item[Double blackbody] Two unmodified blackbodies, resulting in four free parameters (2xtemperature, 2xradius).
\end{description}
...
\end{tabular}
\end{center}
\caption[Double BB fit priors]{Priors for the double blackbody minimization. $T_1$ and $T_2$ are the temperatures of the two blackbodies, while $S_1$ and $S_2$ are the `Scale' parameters (which can be converted into radii.)}
...
The best fit values from all three epochs, as well as their uncertainties (\SI{68}{\percent} uncertainty level) can be seen in Table \ref{tab:double_bb}.
...
\begin{figure}[htb]
    ...
    \caption[Uncertainty estimation for double BB fit]{Uncertainty estimation for the double blackbody fits. Here, the uncertainty of the temperature $T_1$ of the hot (`blue') blackbody during epoch 1 is estimated. The blue points show the \SI{68}{\percent} confidence level.}
    \labfig{bb_chisq}
\end{figure}
