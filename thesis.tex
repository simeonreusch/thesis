...
By integrating $p(\theta)$ and using \ref{delta_e_over_e}, we can calculate the average energy gain of each shock crossing:
...
We already know that on average $\Delta E/E \simeq v_s/c$. $v_s$ can be translated to $\frac{4}{3} w$ where $w$ is\todo{bla} the speed of the shock seen from.


 % so -- on average -- $\Delta E/E = \frac{4}{3}\frac{v_p}{c}$. Then, $\beta = E/E_0 = 1 + \frac{4}{3}\frac{v_p}{c}$.\todo{explain}
...
Both short and long GRBs have been proposed as potential sources of high-energy neutrinos. For 
