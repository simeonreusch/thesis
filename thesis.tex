...
\begin{marginfigure}
    ...
    \caption[Track event in IceCube]{Cascade event: The long track allows for good angular reconstruction, with high uncertainty on the event energy.}
    \labfig{ic_track}
\end{marginfigure}
\begin{marginfigure}
    \includegraphics{ic_cascade.png}
    \caption[Cascade event in IceCube]{Cascade event: The energy is fully contained in the detector, as the event is relatively isotropic. The angular uncertainty is quite large though.}
    \labfig{ic_cascade}
\end{marginfigure}
Track events (Fig. \ref{fig:ic_track}) are produced by secondary muons resulting from the charged-current interaction of $\nu_\mu$ with Antarctic glacial ice\todo{ref to theory section}. They leave tracks in the ice with a length on the order of kilometers. This allows for a good angular resolution, ranging from \SI{1}{\degree} for a \SI{1}{\TeV} muon to \SI{0.3}{\degree} for a \SI{1}{\peta\eV} muon \sidecite{Abbasi2022}. The drawback is a large energy uncertainty, as part of the muon track can lie outside the instrumented volume \sidecite{Aartsen2017a}.
...
Here, $p(x_i|\vec{a})$ is the probability density function (PDF) of measuring $x_i$ given a set of parameters $\vec{a}$. Maximizing $\mathcal{L}$ (or, for technical reasons, minimizing $-\log{\mathcal{L}}$) results in the most likely set of unknown parameters $\vec{a}$ and we are done with the reconstruction. To simplify matters, we assume that we are dealing with a muon with maximum allowed speed ($\beta=1$), travelling along a track of infinite length. The parameters needed to describe this are visualized in Fig. \ref{fig:ic_reco_cherenkov}: $\vec{a}$ = ($\vec{r_0}$, $\vec{p}$, $t_0$, $E_0$), parameterizing the trajectory of a muon at time $t_0$ with energy $E_0$ at position $\vec{r_0}$, traveling in the direction $\vec{p}$.

Now, a DOM in the detector at position $\vec{r}_i$ with a distance $d$ to the track can be hit by Cherenkov photons emitted by the muon with the Cherenkov angle $\theta_\text{C}$.

\begin{figure}
    \includegraphics[width=0.7\textwidth]{amanda_reco.pdf}
    \caption[Angular reconstruction in IceCube]{Bla. Adopted from \cite{Ahrens2004}.}
