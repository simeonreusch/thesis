...
As can be seen in Fig. \ref{fig:agn_unification}, these systems are thought to contain a SMBH in the center, surrounded by an accretion disk of hot material, enshrouded in a dusty torus. Perpendicular to that accretion disk, often jets of relativistic material are launched. When one looks directly into the jet, the system appears as \textit{blazar}. If one moves away from the jet direction, the AGN will in general appear as Type I AGN, displaying broad emission lines stemmming from the so-called \textit{Broad-Line Region} (BLR), lying at a distance of \SIrange{e-2}{1}{\parsec} to the SMBH. It consists of 

Moving further down, the BLR close to the core gets hidden behind the dust torus, and only the  \textit{Narrow-Line Region} (NLR) is still visible, as it is more distant from core (\SIrange{e2}{e4}{\parsec}). Here, gas heated by the core radiates; from the line-broadening one can infer velocities of \SI{\sim 500}{\kilo\m\per\s} \sidecite{Beckmann2012}.
