...
As mentioned above, neutrinos from the sun have already been detected in the 1960s. But only with the first extragalactic neutrino source the era of neutrino astronomy truly began. This source was SN1987a, a supernova in the Large Magellanic Cloud (LMC) that occured in February 1987. 
...
The measured rates were in agreement with theoretical models for core-collapse supernovae, assuming that \SI{99}{\percent} of the supernova energy is released in the form of neutrinos.
...
\begin{figure}[htb]
    ...
    \caption[Core-collapse supernova]{Schematic of a core-collapse supernova. Fusion stops in the core (b), causing it to collapse until neutron degeneracy prevents further collapse. The released energy is carried away by neutrinos (c). The infalling material bounces back from the degeneracy border and forms a shock front (d). Adopted from \url{https://commons.wikimedia.org/wiki/File:Core_collapse_scenario.png}.}
    \labfig{at2019fdr_light_curve}
\end{figure}

During a CCSN, temperatures of \SI{1e11}{\K} are reached. The environment of the core is dense and optically thick. Because of this, photons are unable to escape, as they immediately produce electron-positron pairs. These in turn are quickly absorbed by the surrounding matter. The large binding energy released in the core of \SI{\sim1e53}{\erg} is carried away by neutrinos. Through the annihilation of electron-positron pairs mediated by the $Z$ boson ($e^- + e^+ \rightarrow Z \rightarrow \nu + \bar{\nu}$ \sidecite{Grupen2005}), neutrinos of all flavors are formed and escape the supernova with an average energy on the order of \SI{10}{\mega\eV} \cite{Giunti2007}.

\begin{marginfigure}
    \includegraphics{theory/sn_shells.pdf}
    \caption[CCSN shells]{CCSN shells.}
    \labfig{sn_shells}
\end{marginfigure}

Certain types of CCSNe are thought not only to produce low-energy, but also high-energy neutrinos. For this, one first has to understand the spectral differences between CCSNe types Ib, Ic and II. These can be explained by the presence or absence of the two outermost shells: hydrogen and helium (see Fig. \ref{fig:sn_shells}). If both shells are already far away from the star, the SN is of type Ic. If only the helium shell remains, the type is Ib, and if both shells are still present, the collapse will result in a type II supernova.
