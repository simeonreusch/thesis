...
% \section{Production of Astrophysical Neutrinos}
% Before one can discuss possible sources of astrophysical neutrino, one needs to discuss possible production mechanisms for them. Some of these are intimately linked to the production of Cosmic Rays, so we will shortly discuss them as well.
...
\end{equation}
Therefore, the resulting differential energy spectrum has a spectral index of 2\todo{why not 2.5 (Longair2011) and highlight experimental value of 2.5 above}.
% The energy of a particle that has an initial energy $E_0$ and gains a fixed energy gain fraction $\beta$ that will increase exponentially over time with 
% \begin{equation}
% \label{eqn:e_of_t}
%     E(t) = E_0 \beta^{t/\tau_\beta} = E_0 e^{t/\tau_\beta \ln \beta}
% \end{equation}
% if $\tau_\beta$ is the shock-crossing rate. Assume that each particle has a constant probability of escaping the system, with an average time until it escapes $\tau_\text{esc}$. From this it follows that a particle survives with exponentially decreasing probability 
% \begin{equation}
%     P(t) = \frac{1}{\tau_\text{esc}}e^{-t/\tau_\text{esc}}
% \end{equation}
% To obtain an energy spectrum, one needs to substitute the time $t$. This can be done by rearranging equation \ref{eqn:e_of_t} and solving for $t$:
% \begin{equation}
% t = \tau_\beta \frac{\ln E/E_0}{\ln \beta}.
% \end{equation}
% With this, $P(E)$ can be written as
% \begin{equation}
%     P(E) = \frac{1}{\tau_\text{esc}}e^{-\tau_\beta \frac{\ln E/E_0}{\ln \beta ~\tau_\text{esc}}} = \frac{1}{\tau_\text{esc}}~\frac{E}{E_0}e^{ \frac{-\tau_\beta}{\tau_\text{esc}~\ln \beta}}
% \end{equation}
% The result is a power law with index $\frac{\tau_\beta}{\tau_\text{esc}\ln \beta}$, which is indeed what was already stipulated by experimental data.
...
The energy of the pion decay products is expected to trace the initial proton energy spectrum, so the resulting neutrino spectrum will have \todo{warum?, ergänzen, source}

\textbf{$p\gamma$ interactions}
Cosmic ray protons can also interact with radiation fields. The simplest form of these interactions is the $\delta^+$ resonance, and takes the form
\begin{equation}
    \Delta^+ ~~~ \contour{black}{$\rightarrow$} ~~~ d
\end{equation}
