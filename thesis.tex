...
Today we assume that the myriad of different types of active galaxies (e.g. blazars, Seyfert I and II galaxies, radio-loud galaxies, etc.) are just phenomenologically different. The are a function of the viewing angle, at which we look at the AGN system.

As can be seen in Fig. \ref{fig:agn_unification}, these systems are thought to contain a SMBH in the center, surrounded by an accretion disk of hot material, enshrouded in a dusty torus. Perpendicular to that accretion disk, often jets of relativistic material are launched. When one looks directly into the jet, the system appears as \textit{blazar}. If one moves away from the jet direction, the AGN will in general appear as Type I AGN, displaying narrow emission lines stemmming from the so-called \textit{Narrow-Line Region} (NLR), lying at a distance of \SIrange{e2}{e4}{\parsec} from the SMBH. Here, gas heated by the core radiates; from the line-broadening one can infer velocities of \SI{\sim 500}{\kilo\m\per\s}. With a distance of \SIrange{e-2}{1}{\parsec}, \textit{Broad-Line Region} (BLR) lies closer to the core than the NLR. This why it is often still observable when the NLR is already hidden behind the dust torus  \sidecite{Beckmann2012}.

, and finally, when one looks edge-on towards the dust torus, as narrow-line radio galaxies.

\begin{figure}[htb]
    \includegraphics[width=0.7\textwidth]{theory/agn_unification.pdf}
    \caption[AGN]{AGN unification scheme. Depending on the viewing angle towards the AGN system, different source classes emerge. Adopted from \cite{Thorne2022}.}
