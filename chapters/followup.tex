...
\begin{description}
    ...
    \item[CCSNe] We are sensitive to the subtypes of CCSNe that have been proposed as potential high-energy neutrino sources. The contamination by SNe Ia and the need to determine the exact type of CCSN demand spectroscopic follow up.
...
If an alert makes these cuts, one needs to \textbf{check if observations with ZTF are feasible}. To reduce the potential for errors, this is done with assistance by the \texttt{planobs} \sidecite{Reusch2023} tool, developed and maintained by the author to automate the task as much as possible.
...
\begin{description}
    ...
    \item[TNS] The Transient Name Server (TNS)\sidenote{\url{https://wis-tns.org/}} aims at being the repository of astrophysical transients and contains over 100k objects, both classified and unclassified. It is queried to check if the candidate source is a \textbf{known and possibly classified transient}.
...
The possibility to extract flux prior to the first alert detection makes forced photometry especially useful to accept or reject some candidate neutrino sources: If e.g. a stripped-envelope supernova (that is compatible with high-energy neutrino production, see Section \ref{ccsne}) shows an early forced photometry detection days prior to the neutrino arrival, the source can be rejected as candidate (see e.g. Section \ref{SN2020lls}).\todo{bla}
...
We took a spectrum of SN2020lls with NOT/ALFOSC on June 12, 2020 and used the Supernova Identification (\texttt{SNID}) code \sidecite{Blondin2007} to fit spectra of different types supernovae to the spectrum. The best fit spectrum was a \textbf{type Ic supernova} at redshift $z=0.041$, 14 days post peak. The spectrum and the template are shown in Fig. \ref{fig:ZTF20abdnpdo_spectrum}. 
...
A second source candidate that was also coincident with IC200530A, was SN2020lam \sidecite{IC200530A3} with a supernova-like light curve. To obtain a classification, we took a spectrum with NOT/ALFOSC on June 6, 2020. Template fitting with \texttt{SNID} and \texttt{GELATO} \sidecite{Harutyunyan2008} identified the source as a supernova of type IIP (see Fig. \ref{fig:ZTF20abbpkpa_not_spectrum}).
