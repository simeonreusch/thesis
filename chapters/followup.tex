...
As we only have limited telescope time, we apply quality cuts on the high-energy neutrino alerts received via GCN Notices. Additionaly, we do not follow up alerts with uncertainty areas too large, as the significance of potential counterparts would drop drastically. As outlined in Section~\ref{ic_event_selection}, there are two alert streams: \textbf{Gold alerts} with an average purity of \SI{50}{\percent} (\SI{36}{\percent} of all non-retracted alerts), and \textbf{bronze alerts} with an average purity of \SI{30}{\percent} (\SI{64}{\percent}).
