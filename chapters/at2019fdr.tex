...
\begin{figure*}[htb]
    ...
    \caption[\emph{AT2019fdr} light and SED]{Light curve and SED of \emph{AT2019fdr}. The three panels on the top show the double blackbody fits for the three epochs marked in light gray. On the bottom, the ZTF \textit{g}- and \textit{WISE W1}-band light curve is shown (green circles and gray pentagons), as well as the modeled dust echo emission (black dsh-dotted line). The left y-axes show $\nu F_\nu$ (where $F_\nu$ is the spectral flux density at frequency $\nu$), and the right y-axes display $\nu L_\nu$ (with $L_\nu$ being the luminosity at frequency $\nu$). Figure by the author, from~\cite{Reusch2022}.}
