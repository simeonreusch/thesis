...
To obtain a purer (but of course less complete) TDE sample, the visual cuts established in Section~\ref{visual_cuts} were applied. The classification of nuclear sample after application of these cuts can be seen in Fig.~\ref{fig:classification_maghist_cut}. The full list of all 79 transients from \textbf{non-AGN hosts} classified as TDE, including new and already known ones can be found in the Appendix, see Table~\ref{tab:tde_candidates}. In total, \textbf{27 TDE candidates could be added} to the already known and spectroscopically classified TDEs. These were:

\textit{\href{https://ztfnuclear.simeonreusch.com/transient/ZTF18abtnfnq}{ZTF18abtnfnq}}, \textit{\href{https://ztfnuclear.simeonreusch.com/transient/ZTF18accvmgs}{AT2018ibg}}, \textit{\href{https://ztfnuclear.simeonreusch.com/transient/ZTF18acetnxh}{AT2016fqa}}, \textit{\href{https://ztfnuclear.simeonreusch.com/transient/ZTF19aaeukyu}{AT2021aatf}}, \textit{\href{https://ztfnuclear.simeonreusch.com/transient/ZTF19aanlttm}{AT2019cyt}}, \textit{\href{https://ztfnuclear.simeonreusch.com/transient/ZTF19aaprhvf}{AT2019ekr}}, \textit{\href{https://ztfnuclear.simeonreusch.com/transient/ZTF19aayvxgy}{AT2019hrg}}, \textit{\href{https://ztfnuclear.simeonreusch.com/transient/ZTF19aazlrsy}{ZTF19aazlrsy}}, \textit{\href{https://ztfnuclear.simeonreusch.com/transient/ZTF19abqyosy}{ZTF19abqyosy}}, \textit{\href{https://ztfnuclear.simeonreusch.com/transient/ZTF19abrbskk}{AT2019pcl}}, \textit{\href{https://ztfnuclear.simeonreusch.com/transient/ZTF19abzzuef}{AT2019sez}}, \textit{\href{https://ztfnuclear.simeonreusch.com/transient/ZTF19aclocml}{ZTF19aclocml}}, \textit{\href{https://ztfnuclear.simeonreusch.com/transient/ZTF19acssksf}{AT2019uyn}}, \textit{\href{https://ztfnuclear.simeonreusch.com/transient/ZTF20aavoumq}{ZTF20aavoumq}}, \textit{\href{https://ztfnuclear.simeonreusch.com/transient/ZTF20abbpxut}{AT2020kri}}, \textit{\href{https://ztfnuclear.simeonreusch.com/transient/ZTF20abkstjk}{ZTF20abkstjk}}, \textit{\href{https://ztfnuclear.simeonreusch.com/transient/ZTF20abqjebg}{ZTF20abqjebg}}, \textit{\href{https://ztfnuclear.simeonreusch.com/transient/ZTF20acfxnyr}{ZTF20acfxnyr}}, \textit{\href{https://ztfnuclear.simeonreusch.com/transient/ZTF20aclgfji}{AT2020ygl}}, \textit{\href{https://ztfnuclear.simeonreusch.com/transient/ZTF20acpmkls}{ZTF20acpmkls}}, \textit{\href{https://ztfnuclear.simeonreusch.com/transient/ZTF21aanubdr}{ZTF21aanubdr}}, \textit{\href{https://ztfnuclear.simeonreusch.com/transient/ZTF21aaxtlvc}{AT2021ldl}}, \textit{\href{https://ztfnuclear.simeonreusch.com/transient/ZTF21aaxtpty}{AT2021kqp}}, \textit{\href{https://ztfnuclear.simeonreusch.com/transient/ZTF21aazenvp}{AT2021ovg}}, \textit{\href{https://ztfnuclear.simeonreusch.com/transient/ZTF21abkqvdo}{AT2021swi}}, \textit{\href{https://ztfnuclear.simeonreusch.com/transient/ZTF21acojhgu}{AT2021aees}}, and \textit{\href{https://ztfnuclear.simeonreusch.com/transient/ZTF22aaabqko}{AT2022emf}}.

The 24 objects classified as TDEs that \textbf{were lying in AGN hosts} were also evaluated. Unfortunately, visual inspection of these events ruled out all but one (\textit{ZTF18acryjql}) transient. This was almost always possible based on light curve features alone, highlighting \textbf{potentially lacking performance of the classifier}. On the bright side, it shows that the AGN rejection seems to work, as significantly less events (24 vs 79 for the non-AGN hosts) were initially classified as TDEs at that stage.
...
This was done for the non-AGN part of the sample (see top right histogram in Fig.~\ref{fig:classification_maghist_nocut}). Of the 237 objects classified as TDE, 29 were already seen by the classifier during the training stage and were therefore excluded, leaving 208 transients. These were then visually inspected, and rated by the following scheme: `Correct' meant that the transient looked like a plausible TDE, and no contrary evidence --- like a classification based on a spectrum --- existed. `Possible' meant that either the light curve looked ambiguous, or little data was present, but a clear misclassification could be excluded. Finally, `wrong' was reserved for transients where the visual inspection rendered the TDE-classification unlikely, as e.g.~stochastic AGN variability was present, or the TDE fit result looked unsatisfying (for example, unphysically fast rise and fade timescales).

Of the 208 transients thus analyzed, 79 had a correct classification, for 28 a TDE nature was possible, and 101 were rated as wrong, i.e. misclassifications. When one generously rates both `correct' and `possible' as correct classification, a \textbf{false positive rate of \SI{48.5}{\percent} resulted}. If one is more conservative and counts `possible' as misclassification, a false positive rate of \SI{62}{\percent} ensues. In any case, this marks a sharp drop compared to the \SI{90}{\percent} false-positive rate for the augmented BTS test sample (see Section~\ref{confusion_matrices}). This get alleviated a bit by the fact that TDEs which were present during the training phase were excluded -- so the `real' performance might as well be better. One can safely assume that roughly half of the transients classified as TDE are in fact TDEs.
...
Among this final selection were 16 candidate TDEs that showed a strong dust echo, and which were not published so far. These comprise:
...
\section{Conclusion}
Extracting previously unknown TDE candidates from the nuclear sample by using a trained algorithm aided by visual cuts on the one side, and vetting promising dust-echo sources on the other side yielded \textbf{in total 40 new candidates}. This highlighted that even dedicated programs are missing some candidates. If assuming that about half of the candidates are in fact TDEs (in contrast to AGN activity and SNe, see Section~\ref{all_tde_inspection} for a justification of that number), the luminosity function of TDEs will need to be adjusted. This is not exactly straightforward to do, as all the transients are over by now and the window of opportunity to classify them spectroscopically has closed. It might nevertheless be a promising avenue to obtain the missing host redshifts for these transients (of the 27 \texttt{SGBoost}-classified TDE candidates, 4 have a spectroscopic redshift, 12 a photometric redshift and 11 have none). Of the 16 new dust-echo accompanied candidate TDEs and accretion flares, 6 have a spectroscopic redshift, 6 have a photometric redshift and 4 have none; so the amount of spectra needed is reasonable.

One important remaining question concerns the differentiation between clear-cut TDEs and those events that are somewhat ambiguous. The latter are in general more energetic, longer lived and occur in active galaxies. These are exemplified in Fig.~\ref{fig:two_dustecho_examples}, where \textit{ZTF18abtnfnq} is a classical TDE candidate, and \textit{ZTF20abgxlut} is more luminous, longer lived and situated in an AGN. One could hypothesize that both populations do in fact form the ends of a continuum, together constituting the class of `accretion flares'.

Furthermore, a real-time application of the classifier might be a promising avenue. Of course, such an application will have to deal with a significantly reduced amount of data and the fact that the transients are still evolving. This can be done, as~\sidecite{Miranda2022}  have shown for the more general case of identifying transients worthy of early spectroscopic follow up.
