...
\section{Conclusion}
Extracting previously unknown TDE candidates from the nuclear sample by using a trained algorithm aided by visual cuts on the one side, and vetting promising dust-echo sources on the other side yielded \textbf{in total 39 new candidates}. This highlighted that even dedicated programs are missing some candidates. If conservatively assuming that about half of the candidates are in fact TDEs (in contrast to AGN activity and SNe), the luminosity function of TDEs will need to be adjusted. This is not exactly straightforward to do, as all the transients are over by now and the window of opportunity to classify them spectroscopically has closed.
