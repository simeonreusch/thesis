...
Nevertheless, the evaluation without noisified child light curves is probably more meaningful, as \todo{why}
...
Unfortunately, visual inspection of the 24 events within AGN hosts that were identified as TDE can all but one (\textit{ZTF18acryjql}) be ruled out. This was almost always possible based on light curve features alone, highlighting \textbf{potentially weak performance of the classifier}.

\subsection{Conclusions}
The performance of the classifier yields somewhat mixed results.

\begin{description}
  \item[Data quality] In some cases, inspection of transients classified as TDE showed a sparsely sampled light curve, e.g.~\textit{ZTF21aanubdr}~\sidenote{\url{https://ztfnuclear.simeonreusch.com/transient/ZTF21aanubdr}}. A human would recognize that the sparse sampling allows only little conclusions to be drawn, while the decision tree does not bother. It would be beneficial to somehow infer from the data quality the range of conclusions that could possibly be drawn for a specific object.
  \item[Obvious misclassifications] asdf
\end{description}
