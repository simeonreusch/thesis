...
This method, here dubbed ``Noisification'', was developed by Alice Townsend, with significant contribution by the author. The procedure was implemented as Python package. The procedure works as follows:
...
The flux error gets scaled accordingly, but with two additional empirical constant coefficients determined by fitting the flux error as a function of flux. This procedure showed that the error $F_\text{err}$ deviates from the expected $F$, but can be well approximated by $F_\text{err} = F^{1/2}$
