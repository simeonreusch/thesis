...
These hyperparameters can be tuned by running a grid search in the parameter space, each time training with a small subset of the full data to speed up the process. The parameters yielding the best result are then used when training the full model with the complete training set. In this work, the best values for 9 hyperparameters were searched by randomly drawing from a grid of parameters combinations. The grid employed as well as the chosen hyperparameters are shown in the appendix in Table~\ref{tab:xgboost_grid_search}\todo{chosen values bold in appendix}.
...
\section{Visual Cuts}
The traditional method of selecting TDE candidates are \textbf{visual cuts} in the fit result parameter space (see e.g.~\cite{Velzen2019}). To get a handle on the results of the machine learning classification and \textbf{evaluate the discriminatory power} of the features in an independent way, a similar approach was chosen here.
...
\subsection{Cut stages}
\begin{description}
    \item[AGN Veto] Although the BTS is already biased against AGN, it somehow still contains objects which can be ruled out as likely AGN by crossmatching to Milliquas and selecting based on \textit{WISE} colors (see Section~\ref{agn_rejection}).
    \item[Core distance] As the BTS is not selecting nuclear transients per se, this needs to be enforced by
    \item[asd] asd
\end{description}

\subsection{Results}
The final selection of TDE candidates retained \xx of the initial \xx TDE. This translates to a purity of \xx and efficiency of \xx.
...
\begin{figure}[htb]
    ...
    \end{subfigure}
    \caption[BTS selection]{Visual selection of TDEs among the classified objects of the Bright Transient Survey. These are displayed in the rise- and decay time plane, as derived from the TDE fit (see Section~\ref{tde_model}). From top to bottom and left to right, more and more cuts are added.\newline \newline Top row: No cuts applied (only fit success required, left) and the Milliquas-based AGN veto (right).\newline \newline Second row: AGN cut based on \textit{WISE} colors (left) and cut based on the core distance (right).\newline \newline Third row: The \texttt{sgscore} is required to lie below 0.3 (left) and SNe Ia are removed by a diagonal cut in the rise-/decay time plane (right)\newline \newline Fourth row: The temperature and temperature evolution were required to lie between $\log \SI{3.9}{\K}$ and $\log\SI{4.4}{\K}$, as well as $\SI{-100}{\K\per\day} < \Delta T < \SI{150}{\K\per\day}$ (left) and the rise and decay times were required to lie within $\log \SI{0.8}{\day} < \text{risetime} < \log \SI{2.05}{\day}$ and $\log \SI{1.1}{\day} < \text{decaytime} < \log \SI{3.0}{\day}$ \newline \newline Bottom row: Finally, it was required that the TDE model was a better fit than the SALT2 model by requiring the reduced $\chi^2$ of the TDE fit to be smaller than the SALT2 fit's $\chi^2$ (left), and that the transient light curve had exactly 1 flare (right).}
    \labfig{bts_scatter}.
\end{figure}