...
\begin{description}
    ...
    \item[Core distance (2b)] As the BTS is not selecting nuclear transients per se, this needs to be enforced by
    \item[\texttt{sgscore} cut (3a)] Only transients with an \texttt{sgscore} $<0.3$ were allowed in this cut stage.
    \item[SNIa diagonal cut (3b)] \xx
    ...
    \item[$\chi^2$ cut] Only those transients with a TDE fit reduced $\chi^2$ outperforming their SALT2 fit reduced $\chi^2$ were selected here to reject remaining likely SNe Ia.
    \item[Flare cut] Finally, only transient with a single flare coincident in \textit{g}- and \textit{r}-band were allowed to reject likely stochastic AGN activity.
\end{description}

\subsection{Results}
The final selection of TDE candidates retained \xx of the initial \xx TDE. This translated to a purity of \xx and efficiency of \xx.
