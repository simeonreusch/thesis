...
To obtain a purer (but of course less complete) TDE sample, the visual cuts established in Section~\ref{visual_cuts} were applied. The classification of nuclear sample after application of these cuts can be seen in Fig.~\ref{fig:classification_maghist_cut}. The full list of all 79 transients from non-AGN hosts classified as TDE, including new and already known ones can be found in the Appendix, see Table~\ref{tab:tde_candidates}. In total, \textbf{25 TDE candidates could be added} to the already known and spectroscopically classified TDEs.
...
\section{Conclusion}
Extracting previously unknown TDE candidates from the nuclear sample by using a trained algorithm on the one side, and looking for promising dust-echoes on the other side yielded \textbf{in total 38 new candidates}.
...
One important question remaining concerns the differentiation between clear-cut TDE and such events that are somewhat ambiguous. These are in general more energetic, longer lived and occur in active galaxies. These are exemplified in Fig.~\ref{fig:two_dustecho_examples}, where \textit{ZTF18abtnfnq} is a classical TDE candidate, and \textit{ZTF20abgxlut} is more luminous, longer lived and situated in an AGN. One could hypothesize that both populations do in fact form the ends of a continuum, together constituting the class of `accretion flares'.

Furthermore, a real-time application of the pipeline might be possible.
