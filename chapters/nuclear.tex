...
After this, \textbf{redraw the parent flux} from a normal distribution centered around each parent flux value, scaled by its error. Rescale these re-drawn flux measurements with the new redshift $z_\text{child}$. As the flux $F$ scales according to $D_L=\frac{L}{4 \pi F}$ i.e.~with the square of the luminosity distance, one can calculate a \textbf{flux scaling factor} $a$ according to
\begin{equation}
    a = \frac{D_L(z_\text{parent})^2}{D_L(z_\text{child})^2}.
\end{equation}
With this, the flux of the child lightcurve simply is $f_\text{child} = a f_\text{parent}$.

If an \texttt{SNCosmo} model for the transient type exists, apply \textbf{K-correction}. This
