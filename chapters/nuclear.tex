...
Data retrieval was performed with the \texttt{timewise} package~\sidecite{Necker2023a} to download all datapoints available for each source location in the \textit{W1}- and \textit{W2}-band. The tool automatically binned the single exposures of each epoch, spaced in half-year intervals.

Most sources did have infrared counterparts. To obtain a selection of likely dust-echo candidates, an \texttt{AMPEL} package was used, namely \texttt{T2DustEchoEval}, which analyzed the infrared light curves with a Bayesian block algorithm to find periods of flaring activity compared to a baseline (see also Section~\ref{bayesian_blocks} for a discussion of the algorithm utilized for ZTF optical data).
...
The bottom left matrix shows the prediction-normalized values: Each number is the percentage of the predicted classifications actually belonging to the predicted class. For example, \SI{90}{\percent} of all predicted TDE are actually TDE, the other \SI{10}{\percent} are CCSNe in truth. This corresponds to the false positive rate.\todo{check}

The truth-normalized matrix (bottom right) shows the percentage of objects thruthfully belonging to a class correctly classified as such. \SI{60}{\percent} of all TDE were correctly identified as TDE, but \SI{7}{\percent} were misclassified as AGN, \SI{13}{\percent} were wrongly identified as SNe Ia, \SI{13}{\percent} as CCSNe, and \SI{7}{\percent} as stars.
...
Unfortunately, visual inspection of the 24 events within AGN hosts that were identified as TDE can all but one (\textit{ZTF18acryjql}) be ruled out. This was almost always possible based on light curve features alone, highlighting potentially \textbf{weak performance of the classifier}.

\section{Selection by Dust Echo}\label{dustecho_sel}
A final avenue of obtaining interesting, hitherto missed nuclear transients was a selection based on their infrared dust echo. For this,