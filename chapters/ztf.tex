...
The ZTF image processing can be divided into two parts: The science exposures are taken at Mount Palomar, while calibration, extraction of transients and archival storage happens at the Infrared Processing and Analysis Center (IPAC)\sidenote{\url{www.ipac.caltech.edu}} situated on the campus of the California Institute of Technology (Caltech). IPAC ultimately hosts the ZTF images as part of IRSA\sidenote{\url{irsa.ipac.caltech.edu}}, the Infrared Science Archive.
...
\end{equation}
Now we have a \textit{relative} definition of an source's magnitude, but we need an absolute one. In other words, one needs to know what constitutes a magnitude of 0 (the \textbf{zero point} of the magnitude scale). ZTF uses AB magnitudes \sidecite{Oke1983}, which are defined via the \textbf{spectral flux density} $f_\nu$ (\unit{\W\per\m\per\Hz}). In this sytem, the magnitude is a logarithm of the spectral flux density: 
