...
In the re-simulations, the systematic parameters of the antarctic ice used to model photon propagation were varied. Each re-simulated event was fit with \texttt{Millipede}. From difference between the best-fit likelihood and the ground truth of the simulated event can be employed to convert the change in log-likelihood over the map to a confidence level (see Fig. \ref{fig:ic_millipede} for an example result).\todo{Cristina input}
